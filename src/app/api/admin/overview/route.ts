...
export async function GET(request: NextRequest) {
  ...
  // Fetched separately, before the main Promise.all below — needed as
  // an input to the treasury/surplus queries inside it, so it can't run
  // in parallel with them. Every PLATFORM_FEE_USDT ledger entry (the
  // platform's 30%-of-entry-fee cut, net of referral payouts, AND the
  // separate "unclaimed reward tier" surplus credit) is created with
  // refType: "Match", refId: <that match's id> — confirmed by reading
  // every one of the 3 call sites that ever create this balance type
  // (src/lib/referrals.ts's distributeEntryFeeToTreasuryAndReferrals,
  // and both settle/results routes' own unclaimedUsdt credits). That
  // makes "was this fee/surplus revenue generated by a demo wallet's
  // match" a real, traceable question, unlike Available Balance/
  // Withdrawals/etc. above (which read a wallet's OWN balance
  // directly) — Platform Treasury and Unused Prize Surplus land on the
  // treasury wallet, so they need this extra hop through Match ->
  // MatchParticipant to find out whose activity actually earned them.
  // Confirmed live as the explicit ask: "if some user we already
  // marked demo... subtract that data to show actual real data."
  const demoMatchIds = [
    ...new Set(
      (
  ...
  const [
    ...
    // PLATFORM_FEE_USDT (only ever held by the "platform:treasury"
    // pseudo-wallet) IS traceable back to which match earned it — see
    // demoMatchIds' own doc-comment above — so it excludes fee revenue
    // from a demo wallet's matches, same as every real-user figure
    // elsewhere on this page. `refId: null` is kept, not excluded — no
    // PLATFORM_FEE_USDT entry currently exists without a Match refId,
    // but a hypothetical future one that isn't match-tied at all has
    // no demo-match question to ask, so it shouldn't be silently
    // dropped by this filter either.
    //
    // MINING_PROTECTION_RESERVE_USDT stays fully UNFILTERED — unlike a
    // match's fee, this is a pooled reserve fed by EVERY mining
    // contract's daily settlement together (src/lib/mining.ts), not
    // attributable to one match/wallet the way PLATFORM_FEE_USDT is;
    // unwinding "how much of the pooled reserve came from a demo
    // wallet's contracts" isn't a well-defined question the same way,
    // and this balance isn't one of the KPI cards currently in
    // question either way.
      ...
      where: {
        OR: [
          { balanceType: BalanceType.MINING_PROTECTION_RESERVE_USDT },
          {
            ...
            OR: [{ refId: null }, { refId: { notIn: demoMatchIds } }],
          },
        ],
      },
      _sum: { amount: true },
    ...
    // prizePoolUsdt) credited to treasury under its own reason code, by
    // src/app/api/matches/[id]/settle and .../results. Same
    // Match-traceable exclusion as PLATFORM_FEE_USDT above — this
    // reason code is ALSO always refType: "Match", refId: <match id>
    // at both call sites (confirmed by reading each), so a demo
    // wallet's own match no longer counts here either.
